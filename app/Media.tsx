...
export default function Media() {
  return (
    <div className='relative w-full px-20'>
      <div className="w-full border border-[#4EAEE5] flex items-center justify-evenly">
        {medias?.slice(0, 5).map((media, index) => (
          <Link key={index} href={media.url} className='group w-full relative border-r border-[#4EAEE5] py-16'>
            <h3 className='relative text-2xl font-bold text-[#4EAEE5] text-center'>{media.name}</h3>
